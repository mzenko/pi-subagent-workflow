...
test.skipIf(!process.env.RUN_E2E)("child one-liner", async () => {
  // Assert on the delivered result JSON, not the bare word: the prompt echo
  // already puts PONG in the transcript, so matching the word alone passes
  // even when the child never runs.
  const output = await runPi("Call subagent with prompt: Reply only PONG.");
  expect(output.replace(/\\+/g, "")).toContain('"text":"PONG"');
});
...
test.skipIf(!process.env.RUN_E2E)("spawn persists a run and child session", async () => {
  ...
  await runPi("Call subagent with prompt: Reply only PONG.", cwd);
...
test.skipIf(!process.env.RUN_E2E)("child with a narrowed toolset runs to completion", async () => {
  ...
  await runPi('Call subagent with excludeTools set to ["read","write","edit","bash"] and prompt: Reply with exactly the word GRANITE.', cwd);
  const runsDir = join(agentDir, "subagent-workflow", "runs", encodeCwd(cwd));
  const latest = readdirSync(runsDir).sort().at(-1);
  expect(latest).toBeDefined();
...
test.skipIf(!process.env.RUN_E2E)("explicitly requested unresolvable tools fail the child", async () => {
  ...
  const output = await runPi('Call subagent with tools set to ["web_search","fetch_content"] and prompt: Look up the current weather in Malta and reply with it. Then report the outcome.', cwd);
  const runsDir = join(agentDir, "subagent-workflow", "runs", encodeCwd(cwd));
  const latest = readdirSync(runsDir).sort().at(-1);
  expect(latest).toBeDefined();
  const status = JSON.parse(readFileSync(join(runsDir, latest!, "status.json"), "utf8"));
